...
@Component({
      ...
      <div class="table-container" *ngIf="entries.length > 0; else emptyState">
        <table mat-table [dataSource]="entries" class="mat-elevation-z1">
          ...
          <ng-container matColumnDef="date">
            <th mat-header-cell *matHeaderCellDef> Archivé le </th>
            <td mat-cell *matCellDef="let element"> 
               <div class="date-stacked">
                  <span class="main-date">{{element.date | date:'dd/MM/yyyy'}}</span>
                  <span class="sub-time">{{element.date | date:'HH:mm:ss'}}</span>
               </div>
            </td>
          </ng-container>
          ...
          <ng-container matColumnDef="source">
            ...
            <td mat-cell *matCellDef="let element"> 
               <span class="source-badge">{{ element.data_snapshot?.source || 'Rapport Manuel' }}</span>
            </td>
          </ng-container>
  ...
  styles: `
    ...
    table { width: 100%; min-width: 800px; background: var(--surface); color: var(--text-main); }
    
    .date-stacked { display: flex; flex-direction: column; }
    .main-date { font-weight: 600; color: var(--text-main); }
    .sub-time { font-size: 12px; color: var(--text-secondary); }

    .source-badge { font-size: 11px; padding: 2px 8px; background: #f3f4f6; color: #6b7280; border-radius: 4px; border: 1px solid #e5e7eb; }
...
export class HistoryViewComponent implements OnInit {
  ...
  displayedColumns: string[] = ['date', 'source', 'vehicles', 'cost', 'savings'];
